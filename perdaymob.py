...
import ftplib  # Required to write files back to the FTP server
from io import StringIO # Required for in-memory file operations before uploading
import urllib.request # Required to read the JSON file from FTP

# --- 1. INITIAL SETUP & FTP CONFIGURATION ---
st.set_page_config(layout="wide", page_title="Sales Dashboard")

# Securely get FTP credentials from Streamlit secrets
try:
    ...
    # Define paths for files on the FTP server
    FTP_PRIMARY_CSV_PATH = "/public_html/VVD_Hic/data_storage/primary.csv"
    FTP_CREDENTIALS_PATH = "/public_html/VVD_Hic/data_storage/credentials.json"

    # Create full URLs for reading the files
    PRIMARY_CSV_URL = f"ftp://{FTP_USERNAME}:{FTP_PASSWORD}@{FTP_HOST}{FTP_PRIMARY_CSV_PATH}"
    CREDENTIALS_JSON_URL = f"ftp://{FTP_USERNAME}:{FTP_PASSWORD}@{FTP_HOST}{FTP_CREDENTIALS_PATH}"

except KeyError:
    st.error("FTP credentials not found in secrets. Please create and configure your `.streamlit/secrets.toml` file.")
    st.stop()


# --- 2. USER MANAGEMENT FUNCTIONS (FTP-ENABLED) ---

def load_credentials():
    """Loads user data from the credentials.json file on the FTP server."""
    try:
        with urllib.request.urlopen(CREDENTIALS_JSON_URL, timeout=10) as response:
            data = response.read().decode('utf-8')
            return json.loads(data)
    except urllib.error.URLError:
        # This handles cases where the file doesn't exist (returns a 550 error from FTP)
        return None
    except Exception as e:
        st.error(f"Error loading credentials from FTP: {e}")
        return None

def save_credentials(credentials):
    """Saves the credentials dictionary back to the credentials.json file on the FTP server."""
    try:
        # Convert the dictionary to a JSON formatted string
        credentials_str = json.dumps(credentials, indent=2)
        # Create an in-memory text buffer
        buffer = StringIO(credentials_str)
        buffer.seek(0) # Go to the start of the buffer

        # Connect to the FTP server and upload the file
        with ftplib.FTP(FTP_HOST, FTP_USERNAME, FTP_PASSWORD, timeout=10) as ftp:
            # Use storlines to upload the text buffer
            ftp.storlines(f'STOR {FTP_CREDENTIALS_PATH}', buffer)
        return True
    except Exception as e:
        st.error(f"Failed to save credentials to FTP: {e}")
        return False
...
def initialize_credentials_file():
    """If credentials.json doesn't exist on FTP, create it with a default superadmin user."""
    if load_credentials() is None:
        st.warning("`credentials.json` not found on FTP. Creating a new one with a default Super Admin.")
        default_credentials = {
            "usernames": {
                "superadmin": {
                    "name": "Super Admin",
        ...
        if save_credentials(default_credentials):
            st.info("Default Super Admin created on FTP. Username: `superadmin`, Password: `superadmin123`.")
        else:
            st.error("Fatal: Could not initialize credentials file on FTP. Please check permissions.")

# --- 3. DATA LOADING FUNCTION ---
@st.cache_data(ttl=300) # Cache data for 5 minutes
def load_main_data(file_url):
    """Loads the main sales data from the primary.csv file on the FTP server."""
    try:
        df = pd.read_csv(file_url, encoding='latin1', low_memory=False)

        # Original data cleaning steps
        if 'Inv Date' not in df.columns:
            st.error("Data Error: The column 'Inv Date' was not found in the source file.")
            return None
...
def user_management_ui(credentials, df):
    """Renders the User Management page for the Super Admin."""
    ...
    with st.expander("➖ Delete User", expanded=False):
         with st.form("delete_form", clear_on_submit=True):
            user_to_delete = st.selectbox("Select User to Delete", options=[u for u in credentials["usernames"].keys() if u != "superadmin"], key="delete_select")
...
def main_dashboard_ui(df, user_role, user_filter_value):
    """Renders the main dashboard, filtered by user role."""
    ...
    if df.empty:
        st.warning(f"No data available for your assigned filter ('{user_filter_value}'). Please contact the Super Admin if this is incorrect.")
        return
    ...
    # Hierarchical filters based on role
...
# --- 5. MAIN APP: AUTHENTICATION & PAGE ROUTING ---
